...
public class CheckOutForm extends JFrame {
    ...
    public CheckOutForm() {
        ...
        // Layout chính - sử dụng một panel duy nhất cho toàn bộ nội dung
        ...
        // TabbedPane cho phiếu và hợp đồng - cài đặt kích thước ưu tiên
        tabbedPane = new JTabbedPane();
        tabbedPane.setFont(LABEL_FONT);
        tabbedPane.setBackground(BACKGROUND_COLOR);
        tabbedPane.setPreferredSize(new Dimension(850, 300)); // Cài đặt kích thước ưu tiên
        ...
        // Thêm tabbedPane vào phần trên của centerPanel
        centerPanel.add(tabbedPane, BorderLayout.NORTH);

        // Tạo panel thông tin chi tiết và thêm vào phần trung tâm của centerPanel
        JPanel infoPanel = createInfoPanel();
        infoPanel.setPreferredSize(new Dimension(850, 300)); // Cài đặt kích thước ưu tiên
        centerPanel.add(infoPanel, BorderLayout.CENTER);

        // Thêm centerPanel vào contentPanel
        contentPanel.add(centerPanel, BorderLayout.CENTER);

        // Tạo và thêm panel nút vào phần dưới của contentPanel
    ...
    /***************************************************************************/
    // METHOD MỚI: Load danh sách hợp đồng đang sử dụng
    private void loadDanhSachHopDongDangSuDung() {
        modelHopDong.setRowCount(0);
        List<HopDong> list = hopDongBUS.getHopDongDangSuDung(); // trạng thái = 'dang_su_dung'
    ...
    // METHOD MỚI: Hiển thị thông tin hợp đồng khi chọn
private void hienThiThongTinHopDong() {
        int selectedRow = tableHopDong.getSelectedRow();
        if (selectedRow < 0) return;

        // Get maHopDong from the first column
        ...
        // Phân tích ngày trả dự kiến vs ngày hiện tại
        try {
...
// Add this method to CheckOutForm.java
private void loadDanhSachPhieuDangSuDung() {
        // Clear existing data
        modelPhieu.setRowCount(0);

        // Get checked-in tickets from BUS
        ...
        if (danhSach != null && !danhSach.isEmpty()) {
            ...
            if (tablePhieu.getRowCount() > 0) {
                ...
                // Trigger selection event to update related UI components
                hienThiThongTinPhieu();
            }
        }
    }
...
private void hienThiThongTinPhieu() {
        if (tabbedPane.getSelectedIndex() == 0) {
            ...
            // Extract maPhieu from the selected row
            ...
            // Display service details for the selected phiếu
            hienThiChiTietDichVu(maPhieu);
        } else {
            // Tab hợp đồng - use the existing method
            hienThiThongTinHopDong();
        }
    ...
    private void thucHienCheckOutHopDong() {
        ...
        try {
            ...
            // Kiểm tra ghi chú bắt buộc nếu trả sớm/trễ
            ...
            // TODO: Thay thế bằng mã nhân viên đang đăng nhập
            ...
            if (checkOutBUS.themCheckOut(checkOut)) {
                ...
                // Lấy danh sách mã phòng từ chi tiết hợp đồng
    ...
    private void thucHienCheckOutPhieu() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có muốn tạo hóa đơn thanh toán không?", "Tạo hóa đơn", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            new HoaDonForm(maPhieu, false); // false vì là phiếu đặt phòng
        }
    }
...
private void hienThiChiTietDichVu(String maPhieu) {
        ...
        try {
            // Set Nimbus Look and Feel with custom colors
            for (UIManager.LookAndFeelInfo info : UIManager.getInstalledLookAndFeels()) {
                if ("Nimbus".equals(info.getName())) {
                    ...
                    // Ensure colors for buttons and headers
